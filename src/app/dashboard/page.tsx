...
import { listItems, listProjects, listReminders, getStats } from "@/lib/queries";
import {
  Plus,
  FileUp,
  StickyNote,
...
export default async function DashboardPage() {
  const [items, projects, reminders, stats] = await Promise.all([
    listItems({ limit: 6 }),
    listProjects(),
    listReminders(),
    getStats(),
  ...
  return (
    <div className="space-y-6">
      <section className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <div className="field-label">Good evening, Vaibhav</div>
          ...
          <p className="mt-2 max-w-xl text-sm text-ink-mute">
            RecallOS turned today's 38 processed captures into 12 project briefs,
            23 reusable prompts, and 4 job-search actions. Pick a card and ship.
          </p>
        </div>
          ...
          <div className="space-y-3">
            <div className="rounded-xl border border-line-soft bg-bg-soft/40 p-3">
              ...
              <div className="mt-1 text-sm text-ink">
                Promote the SaaS landing prompt to your Prompt Library — used 4x this month.
              </div>
            </div>
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        ...
        <Card>
          ...
          <ul className="space-y-2">
            <li className="rounded-xl border border-line-soft bg-bg-soft/40 p-3">
              <div className="text-sm text-ink">Generate SaaS landing page copy</div>
              <div className="mt-1 flex items-center gap-2 text-xs text-ink-mute">
                <Trophy className="h-3.5 w-3.5 text-warn" />
                Quality 88 · marketing
              </div>
            </li>
            <li className="rounded-xl border border-line-soft bg-bg-soft/40 p-3">
              <div className="text-sm text-ink">Critique my resume bullet</div>
              <div className="mt-1 flex items-center gap-2 text-xs text-ink-mute">
            ...
            <li className="rounded-xl border border-line-soft bg-bg-soft/40 p-3">
              <div className="text-sm text-ink">Generate a project README</div>
              <div className="mt-1 flex items-center gap-2 text-xs text-ink-mute">
                <Brain className="h-3.5 w-3.5 text-success" />
                Quality 86 · engineering
              </div>
            </li>
          </ul>
        </Card>
